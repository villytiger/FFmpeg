...
static int v4l2_request_dequeue_buffer(V4L2RequestContext *ctx, V4L2RequestBuffer *buf)
{
    ...
    return ioctl(ctx->video_fd, VIDIOC_DQBUF, &buffer);
}
...
#endif
#ifdef DRM_FORMAT_MOD_ALLWINNER_MB32_TILED
    V4L2_PIX_FMT_SUNXI_TILED_NV12,
#endif
};
...
static int v4l2_request_set_drm_descriptor(V4L2RequestDescriptor *req, struct v4l2_format *format)
{
    ...
    case V4L2_PIX_FMT_SUNXI_TILED_NV12:
        layer->format = DRM_FORMAT_NV12;
        desc->objects[0].format_modifier = DRM_FORMAT_MOD_ALLWINNER_MB32_TILED;
        break;
#endif
    default:
        return -1;
...
static int v4l2_request_buffer_alloc(AVCodecContext *avctx, V4L2RequestBuffer *buf, unsigned int type)
{
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
